...
public class DataMigrationActivity extends InstanceActivity {
    ...
    private void exportSearchHistory() {
        try {
            ...
            Log.e("DataMigration", "导出搜索记录失败", e);
    ...
    private void importSearchHistory() {
        ...
        importLauncher.launch(intent.putExtra("import_type", 0)); // 0=搜索记录
    }
    ...
    private void importSettings() {
        ...
        intent.setType("application/json");
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        importLauncher.launch(intent.putExtra("import_type", 1)); // 1=设置
    }
    ...
    private final ActivityResultLauncher<Intent> importLauncher =
            registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
                ...
                // 根据启动时传入的类型区分导入内容
                boolean isSearchHistory = data.getIntExtra("import_type", 0) == 0;
                final Uri uri = data.getData();
                if (uri == null) {
                    MsgUtil.showMsg("无法读取所选文件");
                    return;
                }
                ...
                // 在后台线程中处理文件读取，避免ANR
                new Thread(() -> {
                    // 用 try-with-resources 确保流被释放
                    try (InputStream inputStream = getContentResolver().openInputStream(uri)) {
                        if (inputStream == null) {
                            runOnUiThread(() -> MsgUtil.showMsg("文件读取失败: 无法打开文件"));
                            return;
                        }
                        ...
                        runOnUiThread(() -> {
                            try {
                                ...
                                Log.e("DataMigration", "JSON解析失败", e);
                                MsgUtil.showMsg("JSON解析失败：文件格式不正确");
                            } catch (Exception e) {
                                Log.e("DataMigration", "导入失败", e);
                                MsgUtil.showMsg("导入失败: " + e.getMessage());
                            }
                        });

                    } catch (Exception e) {
                        Log.e("DataMigration", "文件读取失败", e);
                        runOnUiThread(() -> MsgUtil.showMsg("文件读取失败: " + e.getMessage()));
                    }
                }).start();
            });
