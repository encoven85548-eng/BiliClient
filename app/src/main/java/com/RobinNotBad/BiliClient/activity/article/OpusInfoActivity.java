        ...
        TerminalContext.getInstance().getOpusById(oid)
                .observe(this, (result) -> result.onSuccess((opus) -> {
                    ...
                    if (opus == null) {
                        ...
                        MsgUtil.err("opus data is null", new NullPointerException("opus data is null"));
                        return;
                    }
                    ...
                    // ✅ 2. 安全获取 reply 数量，若 stats 为 null 则默认 0
                    int replyCount = (opus.stats != null) ? opus.stats.reply : 0;
                    // ✅ 3. 安全获取 up 的 mid，若 upInfo 为 null 则默认 0
                    long upMid = (opus.upInfo != null) ? opus.upInfo.mid : 0L;

                    replyFragment = ReplyFragment.newInstance(opus.commentId, opus.commentType, replyCount, seek_reply, upMid);
                    // ✅ 4. 只有 upInfo 不为 null 时才调用 setManager
                    if (opus.upInfo != null) {
                    ...
                    // ⚠️ 5. 可选：如果 oiFragment.getView() 可能为 null，也可做保护，但概率低，暂不修改
