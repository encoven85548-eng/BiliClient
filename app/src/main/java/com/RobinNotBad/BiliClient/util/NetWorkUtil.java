...
public class NetWorkUtil {
    ...
    public static JSONObject getJson(String url, ArrayList<String> headers) throws IOException, JSONException {
        // get() 已通过 executeWithRetry 按实验室设置自动重试（次数与间隔），此处只需读取并分类
        try (ResponseBody body = get(url, headers).body()) {
            if (body != null) {
                String responseText = body.string();

                // 检查是否是HTML页面（重试后仍为HTML，说明被风控/拦截）
                if (responseText.trim().startsWith("<!DOCTYPE") || responseText.trim().startsWith("<html")) {
                    Logu.e("getJson: 最终仍收到HTML页面而不是JSON数据: " + url);
    ...
    /**
     * 执行带重试的请求（get/post 通用）。
     * 次数与间隔由实验室"网络请求"设置控制；每次重试前按间隔延迟，避免并发负载过载与风控。
     * 若响应为 HTML 风控页（<!doctype 开头），按设定次数重试。
     */
    private static Response executeWithRetry(OkHttpClient client, Request request) throws IOException {
        ...
        for (int attempt = 1; attempt <= maxTimes; attempt++) {
            ...
            try {
                ...
                Logu.e("收到HTML页面而不是JSON数据，重试 " + attempt + "/" + maxTimes + ": " + request.url());
            } catch (IOException e) {
                latestException = e;
            }

            // 延迟访问：每次重试前等待设定间隔，防止并发负载过载与触发风控
    ...
    /** 判断响应是否为 HTML 风控页（peek 前 128 字节检测 <!doctype） */
    private static boolean isDoctypeResponse(Response response) {
        ...
        try {
            ...
            String s = peekBody.string();
            return s != null && s.trim().toLowerCase(java.util.Locale.ROOT).startsWith("<!doctype");
