...
public class MsgUtil {
    ...
    public static void err(String desc, Throwable e) {
        ...
        if (e instanceof IOException) {
            // 网络类错误：附带具体原因，便于定位（不再只显示笼统的“网络错误”）
            output.append(context.getString(R.string.err_network));
            String io_msg = e.getMessage();
            if (!TextUtils.isEmpty(io_msg)) output.append("\n").append(io_msg);
