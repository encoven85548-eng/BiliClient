    ...
    private static Opus getOpusByHtmlParsing(long id, boolean isArticle) throws IOException, JSONException {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                // 注意：opus 页面本身就以 <!DOCTYPE html> 开头返回完整 HTML，
                // 不能据此判定为"错误页"！直接从中提取 detail JSON 即可。
                // 若 detail 提取失败，则优雅降级：标记为旧版动态让 Activity 走 DynamicInfoActivity。

                String detailJson = JsonUtil.search(html, "detail", "");
                if (detailJson == null || detailJson.isEmpty()) {
                    // detail 提取失败，优雅降级为旧版动态页展示
                    Logu.e("OpusApi: detail提取失败，降级旧版动态 type=" + opus.type + " id=" + id);
                ...
                opus.commentId = Long.parseLong(basic != null ? basic.optString("comment_id_str", "0") : "0");
                opus.commentType = basic != null ? basic.optInt("comment_type") : 0;

                if (detail.isNull("modules")) {
                    // 无图动态/特殊动态可能没有 modules，优雅降级为旧版动态页展示
                    opus.type = Opus.TYPE_DYNAMIC_OLD_STYLE;
                    opus.commentId = (opus.commentId == 0) ? id : opus.commentId;
                    opus.commentType = (opus.commentType == 0) ? 17 : opus.commentType;
                    opus.cover = "";
                    return opus;
                }
                ...
                for (int i = 0; i < modules.length(); i++) {
                    ...
                    switch (module.optString("module_type")) {
                        case "MODULE_TYPE_TITLE":
                            JSONObject module_title = module.optJSONObject("module_title");
                            if (module_title != null) opus.title = module_title.optString("text", "");
                            break;
                        ...
                        case "MODULE_TYPE_AUTHOR":
                            JSONObject module_author = module.optJSONObject("module_author");
                            if (module_author != null) {
                                UserInfo author = new UserInfo();
                                author.mid = module_author.optLong("mid", 0);
                                author.name = module_author.optString("name", "");
                                author.followed = module_author.optBoolean("following", false) || module_author.optInt("following", 0) == 1;
                        ...
                        case "MODULE_TYPE_CONTENT":
                            JSONObject module_content = module.optJSONObject("module_content");
                            if (module_content != null) {
                                JSONArray paragraphs = module_content.optJSONArray("paragraphs");
                ...
                if (opus.paragraphs == null) opus.paragraphs = new OpusParagraph[0];
                Logu.e("OpusApi: 解析成功 type=" + opus.type + " pubTime=[" + opus.pubTime + "] title=[" + opus.title + "] paragraphs=" + (opus.paragraphs != null ? opus.paragraphs.length : -1) + " id=" + opus.id);
